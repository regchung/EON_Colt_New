...
DEFAULTS: list[dict] = [
    ...
    {"key": "service_end_hour", "value": "20", "value_type": "int", "group": "派遣規則",
     "label": "服務時段迄(時)", "description": "每日最晚『可上車』時間(上車限 06:00–此時)"},
    {"key": "max_inter_stop_min", "value": "0", "value_type": "int", "group": "派遣規則",
     "label": "趟次間最大空駛(分)", "description": "同一車輛兩趟之間的空駛行車時間上限(分)；0=停用此限制，由 VROOM 自行最佳化"},
    {"key": "welfare_accept_normal", "value": "true", "value_type": "bool", "group": "派遣規則",
     "label": "福祉車可接一般趟", "description": "開啟後，福祉車在服務完福祉單有餘裕時，可自動接一般趟次，提升福祉車利用率"},
    {"key": "max_tasks_per_vehicle", "value": "15", "value_type": "int", "group": "派遣規則",
     "label": "每車每日最大趟數", "description": "單台車每日派遣趟次上限，建議 10-15，防止排班過密"},
    {"key": "vehicle_fixed_cost", "value": "500000", "value_type": "int", "group": "派遣規則",
     "label": "用車固定成本(最少用車)", "description": "每輛車被啟用的固定懲罰成本；值越高 VROOM 越傾向塞滿現有車再開新車（最少用車目標）。預設 500000 ≈ 138 小時行駛當量，遠高於單車日工時，可有效壓低用車數。調低此值可換取更短行駛時間但可能多用車"},
    ...
    {"key": "pickup_window_min", "value": "15", "value_type": "int", "group": "派遣規則",
     "label": "上車最晚延誤(分)", "description": "抵達超過預約時間此分鐘數則不予派趟（VROOM 時間窗上限）"},
    {"key": "pickup_early_min", "value": "15", "value_type": "int", "group": "派遣規則",
     "label": "上車最早提前(分)", "description": "車輛最早可比預約時間提前幾分鐘到達"},
...
def dispatch_params(db: Session) -> dict:
    ...
    return {
        ...
        "pickup_early_min": get(db, "pickup_early_min", 15),
        ...
        "max_inter_stop_sec": int(get(db, "max_inter_stop_min", 15) * 60),
        "welfare_accept_normal": bool(get(db, "welfare_accept_normal", True)),
        "vehicle_fixed_cost": int(get(db, "vehicle_fixed_cost", 500000)),
        "max_tasks_per_vehicle": int(get(db, "max_tasks_per_vehicle", 15)),
        "tdx_traffic_enabled": bool(get(db, "tdx_traffic_enabled", False)),
        "tdx_window_adjust": bool(get(db, "tdx_window_adjust", False)),
    }
