#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""產生 EON COLT 人工 vs 自動 對比報告 PDF(中文)。"""
import json
...
doc = SimpleDocTemplate("/Users/ycchfx/AI實作/EON_COLT/EON_COLT_對比報告.pdf",
                        pagesize=A4, topMargin=18*mm, bottomMargin=16*mm,
                        leftMargin=18*mm, rightMargin=18*mm,
                        title="EON COLT 人工 vs 自動 對比報告")
story = []
...
P("三、方法與流程", h2)
for line in [
    f"<b>1. 資料匯入</b>:{len(by_fleet)} 個車行的長照平台派遣匯出檔,共 {ov[0]} 筆(含訂單與人工派遣結果)。"
    "已去識別化(不儲存身分證號)。",
...
gap(6)
P("註:單日多為單筆訂單的車行,人工與自動皆用 1 車,故差異有限。", small)
gap(8)
...
P("本報告由 EON COLT 系統自動產生。資料來源:小驢駒集團長照派遣歷史(已去識別化)。", small)

doc.build(story)
print("PDF 已產生:/Users/ycchfx/AI實作/EON_COLT/EON_COLT_對比報告.pdf")
