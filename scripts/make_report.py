...
for line in [
    ...
    "<b>4. 引擎</b>:自架 OSRM 計算台灣真實道路行車時間矩陣;VROOM 求解(含福祉車技能、真實共乘座位、"
    "預約時間窗,並令每車路線首站自起點出發、末站返回終點)。",
    "<b>5. 對比方法</b>:逐(車行 × 日)取「已成行」訂單,車隊 = 人工當天實際用到的車,"
...
for line in [
    ...
    "放寬時間窗可進一步消化。",
    "本版已導入車隊名冊的<b>真實可載客數</b>與<b>出車起點/收車終點</b>,共乘判定與首末段里程較前版貼近實況;"
    "尚有部分車輛未在名冊中,仍以預設值估算。",
]:
    P("• " + line)
gap(8)
