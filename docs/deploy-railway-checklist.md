...
```bash
cd /Users/ycchfx/AI實作/SmartCar
docker compose exec -T db pg_dump -U smartcar -d smartcar \
  --no-owner --no-acl -Fc -f /tmp/smartcar.dump
docker compose cp db:/tmp/smartcar.dump ./smartcar.dump   # 取到本機
# ⚠️ smartcar.dump 含真實 PII:勿提交 git、勿放公開位置;傳輸用加密通道。
```
...
- `smartcar.dump`(含 PII)**永不進 git**;用後即刪本機暫存。
