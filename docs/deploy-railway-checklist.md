...
```bash
cd /Users/ycchfx/AI實作/EON COLT
docker compose exec -T db pg_dump -U eon_colt -d eon_colt \
  --no-owner --no-acl -Fc -f /tmp/eon_colt.dump
docker compose cp db:/tmp/eon_colt.dump ./eon_colt.dump   # 取到本機
# ⚠️ eon_colt.dump 含真實 PII:勿提交 git、勿放公開位置;傳輸用加密通道。
```
...
- `eon_colt.dump`(含 PII)**永不進 git**;用後即刪本機暫存。
