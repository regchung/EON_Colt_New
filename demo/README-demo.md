...
```bash
bash demo/make-demo-data.sh     # 匯出整庫 → demo/dr_fish.dump(含真實 PII)
bash demo/package-demo.sh       # 打包 → demo/dr_fish-demo-bundle.zip(已排除大檔)
```
把 `dr_fish-demo-bundle.zip` 用**加密隨身碟**拷到展示機(內含真實個資,勿走公開雲端)。

## 二、展示機 — 前置(一次)
- 安裝 **Docker Desktop**(macOS / Windows 皆可)並啟動。
- 解壓 `dr_fish-demo-bundle.zip`。
...
- `demo/dr_fish.dump` 與 bundle zip **含真實 PII**:已 gitignore,勿提交;展示結束請從展示機刪除。
