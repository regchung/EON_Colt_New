...
## 🔔 啟動主動告知(下次開工先向使用者報這兩項待辦)
1. **共乘端點接進報表/前端**:把 `pool-suggest` / `pool-consent` / `pool-recurring` 的成果
   做成 PDF 報告與對比頁的「**共乘增益**」一節(對車隊展示用)。
2. **徵詢成功率學習**:依路線/時段預測共乘徵詢成功率以排序撥打;**需先累積真實徵詢結果**
...
migration 已到 `0013`;表:vehicles / drivers / orders / address_point / address_alias / route_stop / users
...
2. **🔔 共乘端點接進報表/前端(下次啟動主動告知)**:把 `pool-suggest`/`pool-consent`/`pool-recurring`
   成果做成 PDF 報告 + 對比頁的「**共乘增益**」一節(對車隊展示:18.4%→約 26%、134 組/57 常態對)。
