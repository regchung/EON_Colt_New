...
```
- 前端 http://localhost:8088;後端 http://localhost:8000/docs;登入 `admin` / `admin123`
...
8. **匯入人工班表後的標準流程**(每次匯入訂單/班表後都做):
   ...
   **⚠️ 注意**:用 `POST /history/import` 匯入長照平台匯出格式,訂單 status 預設為 `imported`;
   若 CurrentStatus 非「已轉至正式單」(如「已有距離及時間」),`persist_day` 不會處理。
   需先執行 `UPDATE orders SET status='done' WHERE service_date='YYYY-MM-DD' AND status='imported'`。
...
近期新增(2026-07-09 一批):
**`vehicle_source`(車輛來源)欄位**:vehicles 新增 `vehicle_source VARCHAR(10)`(獎助/特約)[0032];
`fleet_import.py` 已支援讀取「車輛來源」欄;`reconcile_fleet` 對帳時同步更新。
**三段式優先派遣**(`comparison.persist_day`):
Pass A 獎助車優先 → Pass B 特約補充(同車行未派單) → Pass C 鄰近特約閒車支援(獎助不跨區借出);
`compare_day` 新增 `vehicles_override`/`order_ids_filter`/`_merge_pass()` 支援多段合併。
**共乘衝突修正**(`dispatch_board._board_auto`):共乘對(`pool_pairs`)不觸發衝突標紅;
派遣板每趟新增「共」badge 標示共乘趟次。
**每日出勤名冊上傳**(`daily_roster_import.py` + `POST /fleet/daily-roster?service_date=`):
上傳當日 XLS → 名冊內車寫 `shift_exception(available=True)`+更新座位/輪椅/vehicle_source+寫 `driver_vehicle_assignment`;
名冊外 active 車一律 `shift_exception(available=False)`(當日停派);不動主 `suspended` 旗標;冪等(每次清舊再寫)。
前端新增「📤 每日出勤名冊」頁(`DailyRoster.vue`):行事曆日期選擇 + 上傳 + 出勤/停派雙欄列表。
**長照平台匯出格式匯入**:現有 `POST /history/import` 已支援本格式(Excel serial 日期 pandas 自動轉字串);
匯入後需將 `orders.status` 從 `imported` 改為 `done` 才能被 `persist_day` 納入排班。
**XLS 中文讀取**:Big5 舊格式用 `xlrd(encoding_override='big5')`;新版用 `pd.read_excel(engine='xlrd')`。
**車輛座標初始化**:新匯入車輛若無 start_lng/lat,VROOM 會拋 VroomInputException;
以車行設預設值(台北=121.5654,25.0330;新北/神同行=121.4628,25.0168),因中文 WHERE 條件在 PowerShell 會遇編碼問題,改用 `encode(home_fleet::bytea,'hex')` 比對。

近期新增(派遣原則 + 車隊治理一批,皆入庫綠燈、69 passed):
**派遣原則更新**(見 `docs/dispatch-principles.md`):①固定趟先派+**可共乘**+**綁指定駕駛**(姓名比對 driver_resolve 解當日車,非綁死車號)②同區同車/≤1h 軟性(VROOM 目標自然達成,不設硬牆)③福祉**只看車型**(`vehicle_type=='welfare'`,不以 need_wheelchair 判定;importer/comparison/dispatcher 全鏈一致)④司機**05:30–18:30 出勤緩衝**(`driver_margin_min` 設定,提早發車接早單)⑤只派給**當天有司機**的車(`_vehicles_with_driver` 過濾)⑥**固定趟只比對乘客姓名、不比對地址**(避免某地點全灌單一司機)。
**車隊治理**:drivers/vehicles **`suspended`(停派)**旗標 + 前端切換鈕(`POST /{vehicles,drivers}/{id}/suspend`);**依名冊對帳**(`fleet_import.reconcile_fleet` + `POST /fleet/reconcile`:不在名冊→停派、名冊內→啟用 + 更新座位/輪椅數);**派遣排除停派**車/司機。vehicles 新增 `wheelchair`(可載輪椅數,名冊「輪椅數」回填)。
**即時派遣實證**:6/22 207 單→26 車、6/23 240 單→28 車,皆 **100% 全派**(用名冊內 30 台真實啟用車)。

近期新增(模型校準 + 逐車對比一批,皆入庫綠燈、82 passed):
**逐車對比**(`comparison.compare_day_by_vehicle` + `/dispatch/comparison/by-vehicle` + `VehicleComparison.vue`):
某日某車行,左人工/右自動**並排**(同一組實體車牌),標差異(換車/換入)、顯示**真實停靠序**(交錯上/下車+到點+在車人數)、駕駛+乘客名、每車總里程/工時。`/comparison/available-days` 供日期選單(不依賴批次)。
**班表匯入**(`schedule_import.py` + `POST /history/import-schedule` + 訂單頁「📋 班表匯入」):人工派遣結果檔(民國日期、地址簿優先地理編碼、假單略過、取代當日)→ 建 done 訂單 + SERVED 派遣歷史。
**🌟 最長乘車時間上限**(原則7):下車窗 ≤ 上車窗末 + max(40分, 直達×1.8+30分),防共乘久載(實例修掉「載 7.6h」);設定 `max_ride_factor`/`max_ride_grace_min`;即時+對比皆套。
**🌟 每趟工時歷史校準**(原則8;`calibration.py` + `fleet_calibration` 表 + `/dispatch/calibration` + 設定頁卡):背靠背連續趟反推每趟真實作業(扣車程+空車),依**車行×福祉**校準(全域一般19.4/福祉21.5分),樣本不足退全域;距離由 OSRM 動態、區域差異由分車行吸收(速度係數~1.0)。取代全域固定40分。
**未派計數修正 + 可行動分類**:`vroom_unassigned` 改訂單級(原 2× 灌水);原因新增 `suspect_geocode`(座標離營運區>40km)/`fleet_saturated`(全車隊滿載)/`solver_margin`(求解邊際)。
**振興醫院座標校正**:Map8 曾把「北投區振興路45號」誤編到臺中市(差110km)→校正+別名重指向。
> 完整推導見 **`docs/findings-calibration-2026-06-23.md`**(發現報告)。

近期新增(既定區塊 + 省車旋鈕 + 報表匯出一批,皆入庫):
**🌟 固定趟既定區塊**(原則9;`orders.occupancy_min`[0027]):occupancy 有值→以唯一技能**釘指定車**、整趟佔用時間為服務時長、**各釘各車不互併**;正常單填空檔。修掉「校車被當散單亂併」(6/23 校車 17→10 失真 → 既定區塊 17→16)。詳見 `docs/findings-fixed-route-2026-06-23.md`。
**固定行程逐條維護**(`fixed_route` 加起迄/車牌/起始時段/佔用時間/乘客數/車型/輪椅/可併[0028]):前端「固定行程」表單可逐條設;缺項回退參數設定預設(`fixed_route_blocks.resolve_params`,佔用時間 OSRM 頭尾+多站緩衝估算)。
**🌟 省車率主旋鈕**(`service_time_factor` 設定):每趟作業=校準×係數(**不改校準真值**),與上車窗同為省車鬆緊主旋鈕;研究證實省車率幾乎全由「時間間距」決定(工時上限/最長乘車/緩衝在真實需求下幾乎無作用),做出 8%–60% 對照轉盤。**系統預設定在 ~14% 省車**(窗9/係數1.55,保守貼近人工);`compare_day`/`run_batch`/`dispatcher` 皆讀此二參數。
**派遣無班表退路**:未建 roster 時改用「全 active 且有司機之車」(原保守視為全不出勤)→ 避免空名冊大量未派(6/24:16車35未派 → 35車2未派,2 筆為福祉單被指定到一般車之資料矛盾)。
**派遣表 Excel 匯出**(`dispatch_export.py` + `GET /dispatch/export` + 報表頁卡):挑日期+車行+版型(單檔多分頁 / 每車一張工作表),authed blob 下載。
**真實資料現況**:已匯入雙北/基隆/神同行歷史 ~4.7 萬單(2024-11~2026-06)+ 6/22–24 班表/訂單;6/24 自動派遣 248→246 派(35 車,OSRM)。

migration 已到 `0032`;表:vehicles / drivers / orders / address_point / address_alias / route_stop / users
...
**`vehicle_source`(車輛來源:獎助/特約,[0032])**;
drivers 新增 **`suspended`(停派)**;orders 新增 `pool_consent_at`/`pool_consent_by`、`case_tag`(個案/地標標籤)、
**`occupancy_min`(固定趟整趟佔用/既定區塊標記[0027])**;**fixed_route 新增起迄/車牌/起始時段/佔用/乘客數/車型/輪椅/可併維護欄[0028]**。
app_settings 新增 **`service_time_factor`(省車鬆緊)** + 固定行程群組參數。
...
> **563 天中自動 511 更省、52 打平、0 落敗**(從不輸人工)。未派 15(訂單級,0.04%,全在台北尖峰/長程)。
> 台北 ↓39.0%、新北 ↓32.5%。詳見 `docs/findings-calibration-2026-06-23.md`。
> **↑此為校準真實值**(原 ↓18.7% 因每趟固定 40 分嚴重高估、未派 2× 灌水,屬保守下限);
> 校準把 40→~20 分(歷史實證),同時加乘車上限防久載、未派改訂單級——非放寬約束硬衝。
> (演進:預設座位/自由共乘/無工時約束時 ↓47–51%,屬理論上限。)
> **↑以上為 `service_time_factor`=1.0(校準真值)之省車率;省車率現為可調旋鈕**
> (時間間距:作業係數 + 上車窗,8%–60% 對照轉盤),**系統預設已改保守 ~14%**(係數1.55/窗9),見原則文件。
> 資料現況為**乾淨真實歷史**(4 車行、~2 萬筆;測試資料已清),可重匯(Downloads 的 .xls + 名冊 + 班表)。
