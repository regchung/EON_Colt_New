...
migration 已到 `0016`;表:vehicles / drivers / orders / address_point / address_alias / route_stop / users
／**dispatch_history**（人工派遣結果）／**dispatch_comparison**（逐日對比結果）／**pool_projection**（共乘增益投影）
／**app_settings**（系統參數設定 key-value）／**shift_pattern**·**shift_exception**（班表:週期+例外）。
...
4. **🟠 調度員 AI 助理 → 受控寫入**:助理 v1(唯讀建議,Claude tool-use)已上線(見里程碑)。
   ...
   (文件智慧匯入已完成,見里程碑。)
5. **測試**:已補 `settings`/`roster`/`zone_affinity`/`assign`/時區/`doc_ingest`/`assistant` 的 pytest(見已完成,47 passed)。
